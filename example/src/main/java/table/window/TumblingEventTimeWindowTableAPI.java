...
public class TumblingEventTimeWindowTableAPI {
    public static void main(String[] args) throws Exception {
        ...
        DataStream<Rate> stream = env.addSource(new BoundedRateGenerator())
                ...
                .assignTimestampsAndWatermarks(
                        WatermarkStrategy
                                .<Rate>forBoundedOutOfOrderness(Duration.ofMillis(0))
                                .withTimestampAssigner((event, timestamp) -> event.timestamp)
                );

        Table table = tableEnv.fromDataStream(stream, $("id"), $("timestamp").rowtime());
